    ...
    const user = await Usuario.findOne({
      ...
      include: [
        ...
        },
        {
          ...
          where: { activo: true },
          include: [
            { model: Empresa, where: { activo: true } },
            { model: Area, where: { activo: true } },
          ],
        },
      ],
    });
...
exports.list_usuarios_responsables = async (req, res) => {
  try {
    const whereClause = {};
    const { idArea } = req.query;
    if (idArea) whereClause["$liderProcesos.id_area$"] = idArea;

    const perfilResponsable = await Perfil.findOne({
      where: {
        strCode: strCodesPerfil.RESPONSABLE,
      },
    });
    whereClause["$perfilUsuarios.id_perfil$"] = perfilResponsable.idPerfil;
    const data = await Usuario.findAll({
      attributes: { exclude: ["contrasenna", "token"] },
      include: [
        {
