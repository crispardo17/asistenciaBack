...
const {
  ...
  getNotificacionesRhHh,
  getNotificacionesResponsable,
...
exports.getNotificacionesByTipo = async (
  tipo,
  idUsuarioRequest,
  transaction
) => {
  try {
    let resp = null;
    switch (tipo) {
      case TIPOS_PARAM_NOTIFICACION.ADMIN:
        resp = await getNotificacionesAdmin(transaction);
        break;
      case TIPOS_PARAM_NOTIFICACION.RRHH:
        resp = await getNotificacionesRhHh();
        break;
      case TIPOS_PARAM_NOTIFICACION.RESPONSABLE_PROCESO:
        resp = await getNotificacionesResponsable(idUsuarioRequest);
        break;
      default:
        resp = await ReporteNotificacion.findAll({ raw: true });
        break;
    }
    return resp;
  } catch (error) {
